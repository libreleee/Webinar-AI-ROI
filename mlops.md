...
6. **피처 엔지니어링 (Feature Engineering)**: 모델이 학습하기 적합한 피처(Feature)를 생성 및 선택합니다.
7. **데이터 분할**: 데이터를 Training, Validation, Test 세트로 나눕니다.

---

## 2. 모델 파이프라인 (Model Pipeline)

모델 파이프라인은 준비된 데이터를 바탕으로 모델을 학습시키고 배포하는 과정입니다.

### 모델 파이프라인 순서
1. **모델 설계 및 구현**: 알고리즘 선정 및 아키텍처 정의.
2. **모델 학습 (Model Training)**: Training 데이터를 사용하여 모델 파라미터를 최적화합니다.
3. **하이퍼파라미터 튜닝**: 성능을 극대화하기 위해 최적의 파라미터 조합을 찾습니다.
4. **모델 검증 (Model Validation)**: Validation 데이터를 사용하여 과적합 여부를 확인하고 성능을 측정합니다.
5. **모델 평가 (Model Evaluation)**: 최종적으로 Test 데이터를 사용하여 모델의 일반화 성능을 확인합니다.
6. **모델 등록 (Model Registry)**: 성능이 확인된 모델을 버전별로 저장소에 등록합니다.
